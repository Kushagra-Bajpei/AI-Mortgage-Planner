...
const openings = [
  ...
  {
    title: 'Financial Product Designer',
    team: 'Design',
    location: 'Remote (Global)',
    type: 'Full-time',
        ...
        <div className="max-w-3xl mx-auto px-4 text-center relative">
          <FadeIn>
            ...
            <p className={`text-xl leading-relaxed ${dark ? 'text-slate-400' : 'text-slate-600'}`}>
              Join a passionate, remote-first team on a mission to revolutionize homeownership with powerful AI mortgage planning.
            </p>
          </FadeIn>
        </div>
      </section>
            ...
            {perks.map(({ icon: Icon, title, desc }) => (
              <StaggerItem key={title}>
                <div className={`p-6 rounded-2xl border text-center ${dark ? 'bg-[#12121a] border-white/8' : 'bg-white border-slate-200 shadow-sm'}`}>
                  <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-emerald-500 to-amber-400 flex items-center justify-center mx-auto mb-4">
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <StaggerChildren className="space-y-4" staggerDelay={0.1}>
            {openings.map(({ title, team, location, type, gradient }) => (
              <StaggerItem key={title}>
                <div className={`group flex flex-col sm:flex-row sm:items-center gap-4 p-6 rounded-2xl border transition-all duration-300 cursor-pointer hover:border-emerald-500/40 hover:-translate-y-0.5 hover:shadow-lg ${dark ? 'bg-[#12121a] border-white/8' : 'bg-white border-slate-200 shadow-sm'}`}>
                  <div className={`w-12 h-12 rounded-xl bg-gradient-to-br ${gradient} flex items-center justify-center flex-shrink-0`}>
                    <span className="text-white font-bold text-lg">{title[0]}</span>
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className={`font-semibold text-lg mb-1 group-hover:text-emerald-400 transition-colors ${dark ? 'text-white' : 'text-slate-900'}`}>{title}</div>
          ...
          <FadeIn delay={0.5} className="mt-12 text-center">
            ...
            <Link
              to="/contact"
              className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-gradient-to-r from-emerald-600 to-amber-500 text-white font-semibold text-sm hover:shadow-[0_0_30px_rgba(5,150,105,0.4)] hover:scale-105 transition-all duration-300"
            >
